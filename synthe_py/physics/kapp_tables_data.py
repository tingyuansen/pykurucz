"""Loader for large KAPP continuum reference tables."""
...
class KappTablesMissing(RuntimeError):
    """Raised when KAPP table archive is missing or incomplete."""


def load_kapp_tables(*, force_reload: bool = False) -> Dict[str, np.ndarray]:
    global _CACHE
    if _CACHE is not None and not force_reload:
        return _CACHE
    if not _DATA_PATH.exists():
        raise KappTablesMissing(
            f"KAPP table archive not found at {_DATA_PATH}. "
    ...
    with np.load(_DATA_PATH, allow_pickle=False) as data:
        ...
        if missing:
            raise KappTablesMissing(
                f"Missing KAPP arrays in {_DATA_PATH}: {', '.join(missing)}"
            )
